...
import {IRequireTestGenerator, IPayloadGenerator, IUtils} from "../interfaces";
...
describe("RequireTestGenerator", () => {
    ...
    beforeEach(() => {
        ...
        when(mockPayloadGenerator.generatePayloadTemplate(anything())).thenReturn(petTemplate);

        requireTestGenerator = new RequireTestGenerator(instance(mockPayloadGenerator), instance(mockUtils));
    });
    
    test("should run scaffold template", () => {
        const expectedCmd = "hygen scaffold new --endpoint /pet --operation post --type require";
    ...
    test("should run positive test template", () => {
        const expectedCmd = "hygen require-test new --endpoint /pet --operation post --name positive --datafile payload-1.json --codes successCodes";
    ...
    test("should write positive payload", () => {
        ...
        verify(mockUtils.writeFileUtil(expectedPath, expectedPayloadString)).called();
    });
});